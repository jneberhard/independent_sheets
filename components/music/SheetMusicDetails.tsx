...
export default async function SheetMusicDetails({ id }: SheetMusicDetailsProps) {
  ...
  return (
    <div className="space-y-12">
      {/* MAIN CONTAINER: Product Image and Core Purchasing UI Metadata */}
      <div className="grid grid-cols-1 gap-8 rounded-2xl border border-[var(--secondary)] bg-white p-6 sm:p-8 shadow-sm md:grid-cols-2">
        ...
        {/* Right Side: Title, Details, Audio Preview & Transaction Actions */}
        <div className="flex flex-col justify-between">
          <div>
            ...
            {/* Repositioned Media Section: Link + Audio Player */}
            {(sheetMusic.externalUrl || sheetMusic.previewMp3Url) && (
              <div className="mt-6 p-4 rounded-xl border border-gray-100 bg-slate-50/50 space-y-4">
                {sheetMusic.externalUrl && (
                  <div>
                    <a
                      href={sheetMusic.externalUrl}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="inline-flex items-center gap-2 rounded-xl bg-slate-800 px-4 py-2.5 text-sm font-bold text-white shadow-sm transition hover:bg-slate-900 active:scale-[0.98]"
                    >
                ...
                {/* Audio Demo Preview embedded next to/below the performance link */}
                {sheetMusic.previewMp3Url && (
                  <div className="space-y-2">
                    <div className="flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-gray-500">
                      <Music className="h-3.5 w-3.5 text-gray-700" />
          ...
          {/* Pricing Box Row Context Container */}
          <div>
            ...
            {/* Primary Action Button Options Footer */}
